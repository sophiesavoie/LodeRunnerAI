...
public class Game {
    ...
    public Game(double screenWidth, double screenHeight, int levelNumber, int msgFlag) {
        ...
        this.ballExplosion = new BallExplosion[100];
    ...
    private void generateMessages(double x, int level) {
        ...
        jetPackMsg = new Message(x * 1.5, 20, "[J]", 22);
        foundJetPackMsg = new Message(x, 400, "You found a Jet Pack!\nPress 'J' to use it", 22);

        winningMsg = new Message(x, 200, completed + CONGRAT_MSG[level - 1], 22);
    ...
    private void makeHundredBalls() {
        for (int i = 0; i < 100; i++) {
            ...
            double r = (Math.random() * (player.getRadius() / 4 - 1)) + 1;
    ...
    public void tick(double dt) {
        ...
        if (!ballsExploding) {
            ...
            for (LevelElement element : level.getEntities()) {
                if (!player.isInvulnerable() && element.intersects(player)) {
